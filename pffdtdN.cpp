...
// #include "plasma.h" // Jeff's original
//The ambient density is a function of position.
#include "plasmaN0.h"    // sharp jump
// #include "plasmaN1.h"     //  cone
// #include "plasmaN3.h"     //  Generic cone
// #include "plasmaN4.h"     //  Asymmetric Cone
...
// Output routines
#include "outputN.h"
// #include "output.h"// Jeff's original
  ...
  if (plasma == 1)
    {
      ...
      df = dt*FREQ_PLASMA; 
//       printf("\t N_0 -> %5.3f, %5.3f, %5.3f 1/cc\n",N_0[i][j][k][l][0]*1e-6,N_0[i][j][k][l][1]*1e-6,N_0[i][j][k][l][2]*1e-6);
    }
  ...
  // Note: C_flag < # is set so that false convergence are overlooked
  while ( Q_flag == 0 )
    {
...
//	}
  ...
  if (fgets(tp1,80,fp1)==NULL)
    return 1;
  b = atoi(tp1);                     //b is the number of cells in the antenna  
  for (a=1;a<=b;a++)
    {
      fgets(tp1,80,fp1);
