...
// Author: Jeff Ward
//Modified
// Changed ambient density values(Asymmetric cone(concentric circles,with varying radius and
...
int Xx = 70,Yy = 70,Zz = 65,strt=1;        //Outer dimensions of the Cone."strt" specifies the starting position of the base of the cone. Xx is the height of the cone,while Zz is the diameter. Keep Xx,Yy,Zz values lesser  than the values for sx,sy and sz(set in dipole.str (in .str)file) respectively.
int boxx,boxy,boxz;
...
////////////////////////////////////////////////////////////////////////////////////////////////
//////////Cone section.
int difx,dify,difz,countt=1,Rot=0;              //Rot is the degrees by which the cone is rotated.
difx = (sx-Xx);
dify = (sy-Yy);
difz = (sz-Zz); 
...
for (i=1;i<=Xx;i++)
  ...
  for (i=strt;i<=boxx+abs(strt)-1;i++)
	...
	theta = (double) 180/pi*atan2(k-c2,j-c1);
//  			printf("theta= %5.3f",theta);
	    for (l=0;l<=2;l++)
	      {
		for (m=0;m<NS;m++)
	         { 	 			
			...
			{
			     N_0[i][j][k][l][m] = (double) N_00[0]*(2+(sin((Rot+theta)*pi/180)))*(R+1)/(Rad+1); ////With decreasing R the density drops from 2No:--- 2/Rad.
			}
		       }     
         	    }   
               }
