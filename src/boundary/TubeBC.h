// Boundary conditions w/ plasma gradient
...
void EBCcalc()
{
  ...
  for (j=1;j<=sy;j++)
	...
	EY[IDX4(1,j,k,tlE_cur)] = EYLEFT[2][j][k][1] + 0.5 * ( EYLEFT[1][j][k][1] - EYLEFT[3][j][k][1] )
	               + ( EYLEFT[2][j][k][2] - EYLEFT[2][j][k][0] );
	EZ[IDX4(1,j,k,tlE_cur)] = EZLEFT[2][j][k][1] + 0.5 * ( EZLEFT[1][j][k][1] - EZLEFT[3][j][k][1] )
	               + ( EZLEFT[2][j][k][2] - EZLEFT[2][j][k][0] );
	// Right NOTE: DYRIGTH[0][.][.][.] = edge
	EY[IDX4(sx,j,k,tlE_cur)] = EYRIGHT[2][j][k][1] + 0.5 * ( EYRIGHT[1][j][k][1] - EYRIGHT[3][j][k][1] )
	                + ( EYRIGHT[2][j][k][2] - EYRIGHT[2][j][k][0] );
	EZ[IDX4(sx,j,k,tlE_cur)] = EZRIGHT[2][j][k][1] + 0.5 * ( EZRIGHT[1][j][k][1] - EZRIGHT[3][j][k][1] )
  ...
  for (i=1;i<=sx;i++)
	...
	{
	  // Front
	  EX[IDX4(i,1,k,tlE_cur)] = EXFRONT[i][2][k][1] + 0.5 * ( EXFRONT[i][1][k][1] - EXFRONT[i][3][k][1] )
	               + ( EXFRONT[i][2][k][2] - EXFRONT[i][2][k][0] );
	  EZ[IDX4(i,1,k,tlE_cur)] = EZFRONT[i][2][k][1] + 0.5 * ( EZFRONT[i][1][k][1] - EZFRONT[i][3][k][1] )
	               + ( EZFRONT[i][2][k][2] - EZFRONT[i][2][k][0] );
	  // Back
	  EX[IDX4(i,sy,k,tlE_cur)] = EXBACK[i][2][k][1] + 0.5 * ( EXBACK[i][1][k][1] - EXBACK[i][3][k][1] )
	                + ( EXBACK[i][2][k][2] - EXBACK[i][2][k][0] );
	  EZ[IDX4(i,sy,k,tlE_cur)] = EZBACK[i][2][k][1] + 0.5 * ( EZBACK[i][1][k][1] - EZBACK[i][3][k][1] )
	...
	{
	  // Bottom
	  EX[IDX4(i,j,1,tlE_cur)] = EXBOTTOM[i][j][2][1] + 0.5 * ( EXBOTTOM[i][j][1][1] - EXBOTTOM[i][j][3][1] )
	    + ( EXBOTTOM[i][j][2][2] - EXBOTTOM[i][j][2][0] );
	  EY[IDX4(i,j,1,tlE_cur)] = EYBOTTOM[i][j][2][1] + 0.5 * ( EYBOTTOM[i][j][1][1] - EYBOTTOM[i][j][3][1] )
	               + ( EYBOTTOM[i][j][2][2] - EYBOTTOM[i][j][2][0] );
	  // Top
	  EX[IDX4(i,j,sz,tlE_cur)] = EXTOP[i][j][2][1] + 0.5 * ( EXTOP[i][j][1][1] - EXTOP[i][j][3][1] )
	                + ( EXTOP[i][j][2][2] - EXTOP[i][j][2][0] );
	  EY[IDX4(i,j,sz,tlE_cur)] = EYTOP[i][j][2][1] + 0.5 * ( EYTOP[i][j][1][1] - EYTOP[i][j][3][1] )
  ...
  for (i=1;i<=3;i++)
      ...
      for (k=1;k<=sz;k++)
	{
	  ...
	  EYLEFT[i][j][k][2] = EY[IDX4(i,j,k,tlE_cur)];
	  EZLEFT[i][j][k][2] = EZ[IDX4(i,j,k,tlE_cur)];
	  ...
	  EYRIGHT[i][j][k][2] = EY[IDX4(sx + 1 - i,j,k,tlE_cur)];
	  EZRIGHT[i][j][k][2] = EZ[IDX4(sx + 1 - i,j,k,tlE_cur)];
	}
	...
	for (k=1;k<=sz;k++)
	  {
	    ...
	    EXFRONT[i][j][k][2] = EX[IDX4(i,j,k,tlE_cur)];
	    EZFRONT[i][j][k][2] = EZ[IDX4(i,j,k,tlE_cur)];
	    ...
	    EXBACK[i][j][k][2] = EX[IDX4(i,sy + 1 - j,k,tlE_cur)];
	    EZBACK[i][j][k][2] = EZ[IDX4(i,sy + 1 - j,k,tlE_cur)];
	  }
	...
	for (k=1;k<=3;k++)
	  {
	    ...
	    EXBOTTOM[i][j][k][2] = EX[IDX4(i,j,k,tlE_cur)];
	    EYBOTTOM[i][j][k][2] = EY[IDX4(i,j,k,tlE_cur)];
	    // Top B.C.
	    EXTOP[i][j][k][0] = EXTOP[i][j][k][1];
	    EYTOP[i][j][k][0] = EYTOP[i][j][k][1];
	    EXTOP[i][j][k][1] = EXTOP[i][j][k][2];
	    EYTOP[i][j][k][1] = EYTOP[i][j][k][2];
	    EXTOP[i][j][k][2] = EX[IDX4(i,j,sz + 1 - k,tlE_cur)];
	    EYTOP[i][j][k][2] = EY[IDX4(i,j,sz + 1 - k,tlE_cur)];
	  }
    }
}
...
void NBCcalc()
{
  ...
  for (i=1;i<=sx;i++)
      ...
      for (k=1;k<=sz;k++)
	{
      ...
      // Top - Bottom NOTE: produces a hot spot in the middel that aproches zero at the edges.
      //  for (i=1;i<=sx;i++) // Commented out to increase speed 
      for(j=1;j<=sy;j++)
	{
	  NE[i][j][6][2] = N_0/20 * ( cos((i-Sloc[1][1])*2*PI/sx) + cos((j-Sloc[1][2])*2*PI/sy) ) +N_0*0.9;
	  if (NE[i][j][6][2] < 0.0)
	    NE[i][j][6][2] = 0.0;
	  NE[i][j][sz-6][2] = N_0/20 * ( cos((i-Sloc[1][1])*2*PI/sx) + cos((j-Sloc[1][2])*2*PI/sy) ) +N_0*0.9;
	  if (NE[i][j][sz-6][2] < 0.0)
	    NE[i][j][sz-6][2] = 0.0;
	}
    }
}
...
void Ninital()
{
  ...
  for (i=1;i<=sx;i++)
	...
	for (l=0;l<=2;l++)
	  {
	    NE[i][j][k][l] = N_0/20 * ( cos((i-Sloc[1][1])*2*PI/sx) + cos((j-Sloc[1][2])*2*PI/sy) ) + N_0*0.9 ;
	    if (NE[i][j][k][l] < 0.0)
	      NE[i][j][k][l] = 0.0;
	  }
