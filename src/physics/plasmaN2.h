...
// Author: Jeff Ward
//Modified
// Changed ambient density values(cone(concentric circles,with varying radius and
// height z)).Works if the Cone is completely inside the bounding box. The position and the size of the cone can be varied.
...
//double N_0[NS];                                 // inital density of species 
...
void PLASMAclear()
{
  ...
  int Xx=60,Yy=70,Zz=65,strt=10;        //Outer dimensions of the Cone."strt" specifies the starting position of the base of the cone. Xx is the height of the cone,while Zz is the diameter. Keep Xx,Yy,Zz values lesser  than the values for sx,sy and sz(set in dipole.str (in .str)file) respectively.
  float r;
...
// printf( " R= %d,s=%d \n",R,s);
    ...
    for (j=dify/2+1;j<=Yy+dify/2;j++)
      for (k=difz/2+1;k<=Zz+difz/2;k++)
	  {
	    for (l=0;l<=2;l++)
	    {
		for (m=0;m<NS;m++)
		{		   
			...
			theta = pi/2;
 			...
 			theta = (float) atan(abs(c2-j)/abs(c1-k));//// Might be used in plasmaN3.h
           	    r= (float) sqrt((j-c2)*(j-c2) + (k-c1)*(k-c1)); //////Distance of present location.	
		    	if (r<=R && r>(R-1)) 	//// Test to determine cells lying within one cell width of R.
			{
			     N_0[i][j][k][l][m] = (double) N_00[0]*2*(R+1)/(Rad+1); ////With decreasing R the density drops from 2No:--- No/2.
			}			
		}		
	      SIG[i][j][k] = 0;
	    }	
