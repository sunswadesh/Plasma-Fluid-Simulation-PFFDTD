# Paper 2 \(\Delta r\) amplitude scan (soft edge)

# Runs oscillating cases at several DeltaR plus needed static brackets.
# Reuses SoftEdge=1. Outputs under results/paper2_delta_r_soft/

param(
  [int]$Rs0 = 4,
  [double[]]$DeltaRs = @(0.5, 1.5, 2.0),
  [double]$PhaseDeg = 0.0,
...
$outbase = Join-Path $root 'results\paper2_delta_r_soft'
...
function Start-ScanCase {
  ...
  return Start-Job -ScriptBlock {
    ...
    $t0 = Get-Date
    "START $(Get-Date -Format s) $Name Sd=$Sd dR=$DeltaR soft=$edgeArg MaxIter=$MaxIter" | Set-Content $logfile
...
$oscCases = @()
$staticSds = New-Object 'System.Collections.Generic.HashSet[int]'
[void]$staticSds.Add($Rs0)
foreach ($dr in $DeltaRs) {
  $rsMax = $Rs0 + [int][math]::Ceiling([math]::Abs($dr))
  [void]$staticSds.Add($rsMax)
  $tag = ('dr{0}' -f (('{0:0.##}' -f $dr) -replace '\.','p'))
  $oscCases += @{ Name = "osc_$tag"; Sd = $Rs0; DeltaR = [double]$dr; Phase = $PhaseDeg }
}
$cases = @()
foreach ($sd in ($staticSds | Sort-Object)) {
  $cases += @{ Name = "static_sd$sd"; Sd = [int]$sd; DeltaR = 0.0; Phase = 0.0 }
}
$cases += $oscCases

$startAll = Get-Date
@(
  "[$($startAll.ToString('s'))] Delta-r soft scan: $($cases.Count) cases MaxParallel=$MaxParallel OMP=$OmpThreads"
  "DeltaRs=$($DeltaRs -join ',') soft=$SoftEdge MaxIter=$maxIter"
) | Set-Content $masterLog

if ($DryRun) {
  $cases | ForEach-Object { Write-Host ("DRY {0} Sd={1} dR={2}" -f $_.Name, $_.Sd, $_.DeltaR) }
  return
}
...
while ($queue.Count -gt 0 -or $running.Count -gt 0) {
  while ($running.Count -lt $MaxParallel -and $queue.Count -gt 0) {
    $c = $queue.Dequeue()
    $outDir = Join-Path $outbase $c.Name
    "[$(Get-Date -Format s)] START $($c.Name) Sd=$($c.Sd) dR=$($c.DeltaR)" | Tee-Object -FilePath $masterLog -Append
    $job = Start-ScanCase -Root $root -ExePath $exepath -StrTemplate $strTemplate `
      -OutDir $outDir -Name $c.Name -Sd $c.Sd -DeltaR $c.DeltaR -Phase $c.Phase `
      -SoftEdge $SoftEdge -F $FrequencyHz -MaxIter $maxIter -Fp $Fp -T $T -VcRate $VcRate -OmpThreads $OmpThreads
  ...
  foreach ($r in $running) {
    ...
    $line = if ($res) { "DONE $($res.Name) exit=$($res.ExitCode) minutes=$($res.Minutes) vc=$($res.VcBytes)" } else { "DONE $($r.Name) (check log)" }
...
Write-Host "Scan outputs under $outbase"
