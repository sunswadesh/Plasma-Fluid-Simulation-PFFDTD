# Paper 2 phase scan at fixed f=700 kHz, soft Delta-r=1
# Oscillating cases only; static brackets reused from paper2_rs_t_pilot_soft.
# Default phases: 0, 45, 90, 135, 180 deg (phi=0 already in soft pilot).

param(
  [int]$Rs0 = 4,
  [double]$DeltaR = 1.0,
  [double]$SoftEdge = 1.0,
  [double[]]$PhasesDeg = @(45, 90, 135, 180),
...
$outbase = Join-Path $root 'results\paper2_phase_scan_soft'
...
function Start-PhaseCase {
  ...
  return Start-Job -ScriptBlock {
    ...
    $t0 = Get-Date
    "START $(Get-Date -Format s) $Name Sd=$Sd dR=$DeltaR phi=$Phase soft=$SoftEdge MaxIter=$MaxIter" | Set-Content $logfile
...
$cases = @()
foreach ($phi in $PhasesDeg) {
  $tag = ('phi{0}' -f ([int]$phi))
  $cases += @{ Name = "osc_$tag"; Phase = [double]$phi }
}

$startAll = Get-Date
@(
  "[$($startAll.ToString('s'))] Phase scan soft: $($cases.Count) osc cases, phases=$($PhasesDeg -join ',')"
  "dR=$DeltaR soft=$SoftEdge f=$FrequencyHz MaxIter=$maxIter MaxParallel=$MaxParallel OMP=$OmpThreads"
) | Set-Content $masterLog

if ($DryRun) {
  $cases | ForEach-Object { Write-Host ("DRY {0} phi={1}" -f $_.Name, $_.Phase) }
  return
}
...
while ($queue.Count -gt 0 -or $running.Count -gt 0) {
  while ($running.Count -lt $MaxParallel -and $queue.Count -gt 0) {
    $c = $queue.Dequeue()
    $outDir = Join-Path $outbase $c.Name
    "[$(Get-Date -Format s)] START $($c.Name) phi=$($c.Phase)" | Tee-Object -FilePath $masterLog -Append
    $job = Start-PhaseCase -Root $root -ExePath $exepath -StrTemplate $strTemplate `
      -OutDir $outDir -Name $c.Name -Sd $Rs0 -DeltaR $DeltaR -Phase $c.Phase `
