...
Analyzer: \texttt{scripts/analyze\_cw\_tu\_partial.py} (last-50\% phasor \(Z\), same convention as the \(\fp\)-screen; series resonance is the first \(\ImZ\) \(+\) to \(-\) crossing). Tables and figures: \texttt{analysis/data/cw\_tu\_*.txt}, \texttt{analysis/figures/cw\_tu\_*.png}.
...
\begin{table}[ht]
\centering
\caption{Selected phasor impedances (\(\si{\ohm}\)). Full table: \texttt{analysis/data/cw\_tu\_summary.txt}.}
\label{tab:cwtu-z}
\begin{tabular}{@{}rrrrr@{}}
\toprule
...
\section{What did not match Tu (2008)}

The Tu observable is the \emph{upward shift} of \(\fres\) with \(\Sd\). That shift was not reproduced:
...
\item If the same crossing definition applies to \(\Sd\ge 2\), the feature has moved \emph{downward} relative to \(\Sd=0\), or the feed impedance is dominated by sheath capacitance without a clear in-band LC crossing.
\end{itemize}

Possible follow-ups (Chapter~\ref{ch:status}): extend the CW grid below 1.50~MHz for \(\Sd\ge 2\); compare against free-space \(\fres\) on the same grid; revisit whether the \(+\) to \(-\) crossing is the right marker when a wide vacuum sheath makes the feed uniformly capacitive in the band.
