...
int zval[sz];					// Points on the z-axis used to calculate the radius. 
float tot=0;
tot=1;
for (i=1;i<=sx;i++)
	{
  ...
  if(i==sx)
	...
	for (k=1;k<=sz;k++)
		...
		for (l=0;l<=2;l++)
		{
			...
			if(j==c1 && k==c2)	    			
			N_0[i][j][k][l][m] = (double)2*N_00[0];	//fake
			else
    ...
    for (j=1;j<=sy;j++)
      for (k=1;k<=sz;k++)
	  {
		...
		for (m=0;m<NS;m++)
		{
		    ...
		    N[i][j][k][l][m] = 0.0;
		    N_0[i][j][k][l][m] = N_0[i][j][k][l][0]*pop[m]; 	    			
           	    r= (float) sqrt((j-c2)*(j-c2) + (k-c1)*(k-c1));							
			if (r<=R && r>(R-1)) 	
			{
			     N_0[i][j][k][l][m] = (double) N_00[0]*2*(R+1)/(Rad+1);
			}
			else if(r>Rad)
			{		     
			    N_0[i][j][k][l][m] = N_00[0];			
			...
			{
			   N_0[i][j][k][l][m] = N_0[i][j-1][k][l][m];	//fake
			}
			else if(N_0[i][j][k][l][m]==0)
			{
			   N_0[i][j][k][l][m] = N_00[0];
			}
		}		
